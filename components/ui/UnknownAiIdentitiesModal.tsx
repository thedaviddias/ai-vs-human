...
import type { AiBreakdownEntry } from "@/lib/unknownAi";

interface UnknownAiIdentitiesModalProps {
  isOpen: boolean;
  onClose: () => void;
  tools: AiBreakdownEntry[];
}
...
export function UnknownAiIdentitiesModal({
  isOpen,
  onClose,
  tools,
}: UnknownAiIdentitiesModalProps) {
  useEffect(() => {
    if (!isOpen) return;
      ...
      <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-900 p-6 shadow-2xl animate-in zoom-in-95 duration-200 sm:p-8">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0">
            <h3 className="text-lg font-bold text-white">Unknown AI Identities</h3>
            <p className="mt-1 text-sm text-neutral-400">
              These identities are grouped under unknown AI assistance. Use this list to add
              explicit mappings.
            </p>
          </div>
        </div>
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="shrink-0 text-right">
                  <div className="text-base font-bold text-white">{formatNumber(tool.commits)}</div>
                  <div className="text-[10px] font-medium uppercase tracking-wider text-neutral-600">
                    Commits
                  </div>
                  <div className="mt-1 text-xs text-neutral-400">
                    {formatNumber(tool.additions)} lines
                  </div>
                </div>
              </div>
            </div>
